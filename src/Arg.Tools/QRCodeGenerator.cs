    ...
    public class QRCodeGenerator : IDisposable
    {
        ...
        private static int GetVersion(int length, EncodingMode encMode, ECCLevel eccLevel)
        {
            ...
            var maxSizeByte = capacityTable.Where(
                x => x.Details
                .Any(y => (y.ErrorCorrectionLevel == eccLevel)))
                .Select(x => x.Details.Single(y => y.ErrorCorrectionLevel == eccLevel).CapacityDict[encMode])
                .Max();
            
            throw new Exception($"Exceeded Max Size: {eccLevel.ToString()}, {encMode.ToString()}, {maxSizeByte} (max bytes)");
        }
