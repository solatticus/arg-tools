    ...
    public static class PayloadGenerator
    {
        ...
        public class WiFi : Payload
        {
            ...
            public WiFi(string ssid, string password, Authentication authenticationMode, bool isHiddenSSID = false)
            {
                ...
                this.ssid = IsHexFormat(this.ssid) ? "\"" + this.ssid + "\"" : this.ssid;
                this.password = EscapeInput(password);
                this.password = IsHexFormat(this.password) ? "\"" + this.password + "\"" : this.password;
        ...
        public class OneTimePassword : Payload
        {
            ...
            private void ProcessCommonFields(StringBuilder sb)
            {
                ...
                var escapedIssuer = string.Empty;
                var escapedLabel = string.Empty;
        ...
        private static bool IsHexFormat(this string inp) 
            => Regex.IsMatch(inp, @"\A\b[0-9a-fA-F]+\b\Z") || Regex.IsMatch(inp, @"\A\b(0[xX])?[0-9a-fA-F]+\b\Z");
    }
}